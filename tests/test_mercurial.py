...
from shelf import Shelf, MercurialShelf, ShelfException
...
class TestMercurial(ShelfTestCase):
    ...
    def test_creating_patch_outside_repository_raises_exception(self):
        ...
        """
        assert_raises(ShelfException, Shelf.create, self.SHELF_PATH)
    ...
    def test_creating_patch_in_subdirectory(self):
        ...
        """
        shelf = Shelf.create(os.path.join(self.REPOSITORY_URI, self.SUB_DIRECTORY_NAME))
        assert_equal(shelf.repository_root, os.path.abspath(self.REPOSITORY_URI))

    def test_shelve_and_apply_change(self):
        """Tests that it is possible to shelve changes in a repository.
        """
        shelf = Shelf.create(self.REPOSITORY_URI)
    ...
    def test_shelve_and_apply_conflicting_change(self):
        ...
        """
        shelve = Shelf.create(self.REPOSITORY_URI)
        ...
        # Create the patch.
        shelve.create_patch(self.PATCH_NAME)
        assert_in(self.PATCH_NAME, shelve.get_patches())
        ...
        # Revert the changes to the file, and apply the patch to see whether we
        # get the expected result.
        shelve.apply_patch(self.PATCH_NAME)
    ...
    def test_shelving_added_file(self):
        ...
        assert_not_in(self.PATCH_NAME, shelve.get_patches())
    ...
    def test_shelving_existing_patch_raises_exception(self):
        ...
        assert_raises(ShelfException, shelve.create_patch, self.PATCH_NAME)
